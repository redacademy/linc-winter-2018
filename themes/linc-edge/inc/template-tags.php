 ...
 */
  ...
  */
 function linc_edge_comment_count() {
  ...
  */
 function linc_edge_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && linc_edge_categorized_blog() ) {
  ...
  * @return bool
  */
 function linc_edge_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'linc_edge_categories' ) ) ) {
 		...
 		set_transient( 'linc_edge_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so linc_edge_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so linc_edge_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in linc_edge_categorized_blog.
  */
 function linc_edge_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'linc_edge_categories' );
 }
 add_action( 'edit_category', 'linc_edge_category_transient_flusher' );
 add_action( 'save_post',     'linc_edge_category_transient_flusher' );
  ...
  */
 function linc_edge_comment_list( $comment, $args, $depth ) {
  ...
  */
 function linc_edge_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
